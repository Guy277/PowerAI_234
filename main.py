...
with tabs[2]:
    ...
    df_display.columns = [
        "Nom", "Âge", "Salaire", "Épargne", 
        "Objectif Financier", "Niveau de Risque", "Statut Bancaire"
    ]
    ...
    # --- NOUVELLE SECTION : GESTION & SUPPRESSION ---
    with st.expander("🛠️ Mode Maintenance - Gestion de la Base de Données", expanded=False):
        st.write("Sélectionnez les clients que vous souhaitez retirer de la base PowerBank.")
        
        # Multi-sélection pour suppression groupée
        to_delete = st.multiselect(
            "Choisir les clients à supprimer :",
            options=df_cleaned['name'].tolist(),
            help="Vous pouvez sélectionner plusieurs noms à la fois."
        )
        
        col_del1, col_del2 = st.columns(2)
        
        with col_del1:
            if st.button("🗑️ Supprimer la sélection", type="primary", use_container_width=True, disabled=not to_delete):
                try:
                    with open(USERS_JSON_PATH, 'r', encoding='utf-8') as f:
                        current_data = json.load(f)
                    
                    # On garde uniquement ceux qui NE sont PAS dans la liste de suppression
                    updated_data = [u for u in current_data if u['name'] not in to_delete]
                    
                    with open(USERS_JSON_PATH, 'w', encoding='utf-8') as f:
                        json.dump(updated_data, f, indent=4, ensure_ascii=False)
                    
                    st.toast(f"✅ {len(to_delete)} client(s) supprimé(s) avec succès.", icon='🗑️')
                    st.rerun()
                except Exception as e:
                    st.error(f"Erreur lors de la suppression : {e}")
        
        with col_del2:
            if st.button("⚠️ VIDER TOUTE LA BASE", use_container_width=True, help="Supprime TOUS les clients du fichier JSON"):
                try:
                    with open(USERS_JSON_PATH, 'w', encoding='utf-8') as f:
                        json.dump([], f, indent=4, ensure_ascii=False)
                    st.toast("Base de données entièrement vidée.", icon='⚠️')
                    st.rerun()
                except Exception as e:
                    st.error(f"Erreur : {e}")
...
with tabs[5]:
    ...
    # Récupération de la palette pour les styles personnalisés
    ...
    with col_stat1:
        ...
        desc_stats = df_cleaned[['age', 'salary', 'savings']].describe()
        desc_stats.index = ['Nombre', 'Moyenne', 'Écart-type', 'Min', '25%', '50%', '75%', 'Max']
    ...
    with col_stat2:
        ...
        **Hypothèse Alternative ($H_1$) :** Il existe une corrélation/association statistique entre l'objectif d'un client et son statut.
        ...
        if p_val < 0.05:
            st.markdown(f"""
            <div style="background-color: rgba(46, 204, 113, 0.15); border-left: 5px solid #2ecc71; padding: 15px; border-radius: 8px; color: {t['text']};">
                <b style="color: #2ecc71; font-size: 1.1em;">✅ Résultat du test : REJET DE L'HYPOTHÈSE NULLE (H₀)</b><br/>
                La p-value (<b>{p_val:.6f}</b>) est inférieure au seuil critique de 5%. Il existe une relation statistiquement significative
                entre la situation financière du client (Statut) et ses projets de vie (Objectif). 
                La segmentation algorithmique de PowerBank est donc <b>mathématiquement justifiée</b>.
            </div>
            """, unsafe_allow_html=True)
        else:
            st.markdown(f"""
            <div style="background-color: rgba(231, 76, 60, 0.15); border-left: 5px solid #e74c3c; padding: 15px; border-radius: 8px; color: {t['text']};">
                <b style="color: #e74c3c; font-size: 1.1em;">⚠️ Résultat du test : NON-REJET DE H₀</b><br/>
                La p-value (<b>{p_val:.6f}</b>) est supérieure ou égale à 5%. Il n'y a pas assez de preuves statistiques pour conclure à
